...
public class RestaurantController {
    ...
    public ResponseEntity<?> saveNewDish(@RequestBody Cuisine cuisine, @PathVariable int restaurantId) throws CuisineAlreadyExistException {
        try{
            return new ResponseEntity<>( restaurantService.addRestaurantCuisine(cuisine,restaurantId),HttpStatus.OK);

        }
        catch (CuisineAlreadyExistException ex){

            throw new CuisineAlreadyExistException();
        }
    }

    @DeleteMapping("/deleteItem/{restaurantId}/{cuisineName}")
    public ResponseEntity<?> delete(@PathVariable("restaurantId") int restaurantId, @PathVariable("cuisineName") String cuisineName) throws RestaurantNotFoundException{
       // return new ResponseEntity<>(restaurantService.getAllRestaurants(), HttpStatus.OK);
        restaurantService.deleteCuisine(restaurantId,cuisineName);
        return new ResponseEntity<>("Cuisine to be deleted "+restaurantId+","+cuisineName,HttpStatus.OK);

    }

    @DeleteMapping("/delete/{restaurantId}/{cuisineName}")
    public ResponseEntity<?> deleteCuisine(@PathVariable("restaurantId") int restaurantId, @PathVariable("cuisineName") String cuisineName) throws RestaurantNotFoundException {
        restaurantService.deleteCuisine(restaurantId,cuisineName);
